...
def process_md_with_langchain(content: str) -> str:
    ...
    ## 处理步骤
    1. **清理符号**: 删除OCR识别错误产生的乱码符号、孤立的特殊字符，但保留正常的标点符号。
    ...
    - **不要**在最终输出的Markdown文本外包裹任何代码块（如 ```markdown ... ```）。
    ...
    **输出：**
    ...
    请按照以上要求处理提供的Markdown文档，确保输出是纯粹的、不被代码块包裹的Markdown文本。
    """
...
def setup_and_process_files():
    ...
    target_dir = os.path.join(knowledge_base_dir, "03_structure_md_files")
